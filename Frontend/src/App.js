...
import { LayoutDashboard, Server, Globe, Menu, X, Terminal, Cpu, ShieldCheck } from "lucide-react";
import "./App.css";
...
export default function App() {
  ...
  return (
    // FIX 1: changed 'min-h-screen' to 'h-screen' and added 'w-full'
    // This locks the app to the viewport height so the sidebar never scrolls away
    <div className="h-screen w-full flex bg-[#020617] text-slate-200 font-sans selection:bg-cyan-500/30 selection:text-cyan-100 relative overflow-hidden">
      ...
      {/* FIX 2: Added overflow-hidden to main to prevent double scrollbars */}
      <main className="flex-1 flex flex-col relative z-10 h-full overflow-hidden">
          ...
          <header className="h-16 border-b border-slate-800 bg-[#020617]/80 backdrop-blur-md flex items-center justify-between px-4 md:px-6 flex-shrink-0">
            ...
            <div className="flex items-center gap-4 text-xs font-mono">
              ...
              <div className="flex items-center gap-2 px-3 py-1 bg-emerald-500/10 border border-emerald-500/20 rounded-full">
                ...
                <span className="text-emerald-400 font-bold tracking-wide">SYSTEM ONLINE</span>
              </div>
            </div>
          </header>
        )}
        ...
        {/* Main routes */}
        {/* FIX 3: This section handles the scrolling. 'flex-1' takes remaining height, 'overflow-y-auto' enables scroll */}
...
function Sidebar({ className = "" }) {
  ...
  return (
    // FIX 4: Ensure sidebar takes full height
    <aside className={`${className} w-72 h-full border-r border-slate-800 bg-[#0B1120] flex flex-col relative`}>
      ...
      <div className="px-6 py-8 border-b border-slate-800/60 bg-gradient-to-b from-slate-900 to-transparent flex-shrink-0">
        <h1 className="text-2xl font-bold tracking-tighter text-white flex items-center gap-2">
          ...
          <span>Uptime<span className="text-cyan-500">Watch</span></span>
        </h1>
      ...
      <nav className="mt-8 px-4 space-y-2 flex-1 overflow-y-auto">
        <SidebarLink to="/" label="Overview" icon={<LayoutDashboard size={18} />} />
        <SidebarLink to="/servers" label="Server Nodes" icon={<Server size={18} />} />
        <SidebarLink to="/websites" label="Endpoints" icon={<Globe size={18} />} />
      </nav>
      ...
      <footer className="mt-auto p-6 border-t border-slate-800 bg-[#050912] flex-shrink-0">
        ...
        <div className="text-[10px] font-mono text-slate-500 space-y-2 border border-slate-800 p-3 rounded bg-slate-900/50">
          ...
          <div className="flex justify-between">
             ...
             <span className="text-cyan-400 hover:underline cursor-pointer">Mizanur.R</span>
          </div>
...
// ... MobileSidebarDrawer and SidebarLink remain the same ...
    ...
    <div className="fixed inset-0 z-50 flex font-sans">
      ...
      <aside className="relative w-72 max-w-[85vw] h-full border-r border-slate-800 bg-[#0B1120] flex flex-col shadow-2xl shadow-cyan-900/20">
        <div className="px-6 py-6 border-b border-slate-800 flex items-center justify-between bg-[#050912] flex-shrink-0">
          ...
          <button className="p-1 rounded text-slate-400 hover:text-white" onClick={onClose}>
            <X size={20} />
          </button>
        </div>

        <nav className="mt-6 px-4 space-y-2 flex-1 overflow-y-auto">
          <SidebarLink to="/" label="Overview" icon={<LayoutDashboard size={18} />} onClick={onClose} />
          <SidebarLink to="/servers" label="Server Nodes" icon={<Server size={18} />} onClick={onClose} />
          <SidebarLink to="/websites" label="Endpoints" icon={<Globe size={18} />} onClick={onClose} />
        </nav>
